function [Lambda,gamma_opt,Psi] = cv_sfm_ts(X,m,Lambda_init,Psi_init,gamma,method)
...
if length(gamma)>1
    ...
    parfor jj = 1:length(gamma)
        [theta_lambda_fold(:,jj),theta_psi_fold(:,jj)] = sfm(X_in,m,Lambda_init(:,:,jj),Psi_init,gamma(jj),method,21,30e3);
    end
    ...
    [param_lambda,param_psi] = sfm(X,m,Lambda_init_opt,Psi_init,gamma_opt,method,51,60e3);
    Lambda = reshape(param_lambda,p,m); Psi = diag(param_psi);
else
    % If gamma is a scalar, no cross-validation
    gamma_opt = gamma;
    [param_lambda,param_psi] = sfm(X,m,Lambda_init,Psi_init,gamma_opt,method,51,60e3); p = size(X,2);
    Lambda = reshape(param_lambda,p,m); Psi = diag(param_psi);
end