...
if length(gamma)>1
    ...
    parfor jj = 1:length(gamma)
        [theta_lambda_fold(:,jj),psi_fold(:,jj)] = EM_algorithm(X_in,m,gamma(jj),1,Lambda_init,Psi_init);
    end
    ...
    [param_lambda,param_psi] = EM_algorithm(X,m,gamma_opt,1,Lambda_init,Psi_init);
