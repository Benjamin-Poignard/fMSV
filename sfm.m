function [param_lambda,param_psi] = sfm(X,m,Lambda_init,Psi_init,gamma,method,iter,maxIt)
...
%          - maxIt: maximum number of iterations (Lambda estimation)
% Outputs:
...
while kk<iter
    ...
    [Lambda_update,param_lambda_update] = penalized_factor_Lambda(X,m,Lambda,Psi,gamma,method,maxIt);

    % Psi-step
    [~,param_psi_update] = psi_estimation(X,m,Lambda_update,Psi);
